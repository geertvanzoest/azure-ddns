...
- [x] **IP-02**: Script valideert het opgehaalde IP-adres met een IPv4 regex check
- [x] **IP-03**: Script vergelijkt het huidige IP met het bestaande DNS record en skipt update als ongewijzigd
- [x] **IP-04**: Script valt terug op een alternatieve IP-service (checkip.amazonaws.com) als de primaire faalt
...
- [x] **DNS-03**: TTL is configureerbaar via environment variable (AZURE_DNS_TTL) met default 300 seconden
...
- [x] **OPS-03**: Script gebruikt flock lock file om concurrent cron execution te voorkomen
- [x] **OPS-04**: Script ondersteunt --force flag om IP-vergelijking te skippen en altijd te updaten
- [x] **OPS-05**: Script ondersteunt verbose modus (VERBOSE=1 env var) voor extra debug output
